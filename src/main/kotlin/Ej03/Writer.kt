...
class Writer(private val path: Path) {
     ...
     * @param listaEmpleados: Listado de los empleados
     * @return fileXML: Una lista mutable de empleados, tras ser modificada en caso de existir dicha ID
     */
    fun editXML(id: Int, newSalary: Double, listaEmpleados: MutableList<Empleado>): MutableList<Empleado> {
        // Find para obtener el empleado con esa ID
        val empleado = listaEmpleados.find { it.id == id }
        if (empleado != null){
            val index = listaEmpleados.indexOf(empleado)
            listaEmpleados[index].salario = newSalary
        }else{
